...
import { construirInputArticulo } from "@/routes/articulos/construirInputArticulo";
...
export function ArticuloEditPage() {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      // Migración orgánica: si el padre traía el campo legacy `descuento`
      // pero no `precioDescuento`, lo subimos al campo nuevo. Al guardar
      // se persiste sólo en `precioDescuento` y `descuento` se vacía.
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!form.nombre.trim() || !form.precioVenta) {
  ...
  return (
    ...
    >
      ...
      <section className="space-y-4 rounded-lg border bg-card p-4 sm:p-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-1.5">
            <Label htmlFor="preciCompra">Precio compra</Label>
            <Input
              id="preciCompra"
              type="number"
              inputMode="decimal"
              value={form.preciCompra}
              onChange={(e) => set("preciCompra", e.target.value)}
            />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="Si > 0 y < base, reemplaza al base"
              />
              {form.subvariaciones.length > 0 && (
                <p className="text-[11px] text-muted-foreground">
                  Aplica como descuento global. Cada subvariación puede
                  sobrescribirlo con el suyo propio.
                </p>
              )}
            </div>
          </div>
