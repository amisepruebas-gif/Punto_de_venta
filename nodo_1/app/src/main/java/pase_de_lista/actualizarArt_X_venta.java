...
public class actualizarArt_X_venta {


    static public void actualizaArticulos_x_venta(String id_subir, Context context, int index) throws JSONException {
        ...
        if (jsonDatos.has(context.getString(R.string.articulos_x_venta))){
            jsonDatos.getJSONObject(context.getString(R.string.articulos_x_venta)).put(id_subir, array_id_dispositivo);
            jsonDatos.getJSONObject(context.getString(R.string.articulos_x_venta)).put("huella", huella);
        }else {
            JSONObject obj = new JSONObject();
            obj.put(id_subir, array_id_dispositivo);
            obj.put("huella", huella);
            jsonDatos.put(context.getString(R.string.articulos_x_venta), obj);
        }
        if (index == 0){
            descarga.unavezArt = false;
            descarga.huellaArticulos_reset = huella;
            fire.documenRef("datos/" + context.getString(R.string.articulos_x_venta)).
                    set(new Gson().fromJson(
                            jsonDatos.getJSONObject(context.getString(R.string.articulos_x_venta)).toString(), HashMap.class)).addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
